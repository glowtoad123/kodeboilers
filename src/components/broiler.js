...
export default function Broiler() {
    ...
    const {id} = useParams()
    const {findBroiler, deleteBroiler, currentUser} = useFire()
    ...
    return (
        <>
            {!broiler && <LinearProgress color="primary" />}
                <div style={{width: '95%', margin: 'auto', marginTop: '50px', backgroundColor: '#263238', borderRadius: '12px 12px 0 0'}}>
                    ...
                    />
                    { broiler && currentUser && currentUser.uid === broiler.user && <><Link to={`/edit/${id}`}>
...
{/*             {broiler && currentUser && currentUser.uid !== broiler.user &&
                        <>
                                <br />
                                <br />
                                <br />
                            <div id="denied">
                                <h1>Access Denied because you are not the creator of this broiler.</h1>
                                <h2>If You are the Creator of this broiler and you are seeing this message, then the Broiler is loading. If you have been waiting forever you may need to login Again.</h2>
                            </div>
                        </>
            } */}
        </>
